...
public class JdbcSelectTest { // Save as "JdbcSelectTest.java"
   public static void main(String[] args) {
      try (
         ...
         // Show books written by author and books below price, sort by highest price.
         // if same price, sort by id.
         String searchAuthor = "Tan Ah Teck";
         double searchPrice = 30;
         String strSelect = "SELECT title, author, price, qty FROM books WHERE author = '" + searchAuthor
               + "' OR price >= " + searchPrice + " ORDER BY price DESC, id ASC";
         ...
         // rset.next() inside the whole-loop repeatedly moves the cursor to the next
         // row.
         ...
         while (rset.next()) { // Repeatedly process each row
            ...
            // int id = rset.getInt("id"); // retrieve an 'int' -cell in the row
